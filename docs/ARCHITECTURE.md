...
### `SD.commands` (commands.js, M2+) — `parse(text)`, `register(def)`, `list()`, `process(msg)`, `handleChat({ username, displayName?, text, source, isMod, ts? }) → { ok, isCommand, command, message, effects, cooldownMs }`; `SD.processCommand(username, text, opts)` alias. Emits `chat:message { id, username, displayName, text, source, isMod, kind:'user'|'reply'|'system', ts }` and `command:result`.
...
## Persistence keys
`spiritderby.save` (state), `spiritderby.backup` (pre-migration/import copy), `spiritderby.ui` (overlay/drawer prefs only).
